...
def get_unit_scale_mesh(origin_mesh:trimesh.base.Trimesh):
    bbox = origin_mesh.bounding_box.bounds
    scale = bbox[1] - bbox[0]
    return origin_mesh.apply_scale(1.0/scale)
...
for it, data in enumerate(tqdm(test_loader)):
    ...
    # Output folders
    if not args.eval_input:
        occ_mesh_dir = os.path.join(generation_dir, 'occnet_meshes')
        ldif_mesh_dir = os.path.join(generation_dir, 'ldif_meshes')
    ...
    if category_id != 'n/a':
        occ_mesh_dir = os.path.join(occ_mesh_dir, category_id)
        ldif_mesh_dir = os.path.join(ldif_mesh_dir, category_id)
    ...
    if cfg['test']['eval_mesh']:
        occ_mesh_file = os.path.join(occ_mesh_dir, '%s.off' % modelname)
        # the number of LDIF mesh files is >= 1, we randomly choose 1 mesh to do evaluation
        ldif_mesh_file = glob.glob(os.path.join(ldif_mesh_dir, modelname, '/*.ply'))[0]
        print(f'ldif_mesh_file: {ldif_mesh_file}')

        if os.path.exists(occ_mesh_file):
            occ_mesh = trimesh.load(occ_mesh_file, process=False)
            eval_occ_mesh_dict = evaluator.eval_mesh(
                occ_mesh, pointcloud_tgt, normals_tgt, points_tgt, occ_tgt)
            for k, v in eval_occ_mesh_dict.items():
                eval_dict[k + ' (occ_mesh)'] = v
            
            ldif_mesh = trimesh.load(ldif_mesh_file, process=False)
            unit_ldif_mesh = get_unit_scale_mesh(ldif_mesh)
            eval_ldif_mesh_dict = evaluator.eval_mesh(
                unit_ldif_mesh, pointcloud_tgt, normals_tgt, points_tgt, occ_tgt)
            for k, v in eval_ldif_mesh_dict.items():
                eval_dict[k + ' (ldif_mesh)'] = v
        else:
            print('Warning: mesh does not exist: %s' % occ_mesh_file)
