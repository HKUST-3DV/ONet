...
# save latent codes
generate_latentcode = cfg['generation']['generate_latentcode']
generate_latentcode = True
...
for it, data in enumerate(tqdm(test_loader)):
    ...
    latentcodes_dir = os.path.join(generation_dir, 'embeddings' )
    ...
    if generate_latentcode and not os.path.exists(latentcodes_dir):
        os.makedirs(latentcodes_dir)
    ...
    if generate_mesh:
        ...
        # Get statistics and laten_code
        try:
            mesh, stats_dict, embedding = out
        except TypeError:
            mesh, stats_dict, embedding = out, {}, {}
        ...
        if len(embedding) > 0:
            # file path: latentcodes_dir/category_id/modelname/emb.npz
            emb_out_folder = os.path.join(latentcodes_dir, category_id, modelname)
            os.makedirs(emb_out_folder, exist_ok=True)
            output_emb = np.array(embedding.squeeze().cpu())
            emb_out_file = os.path.join(emb_out_folder, 'emb')
            np.save(emb_out_file, output_emb)
            print('save latent code {} for {}-{}'.format(embedding.shape, category_id, modelname))
