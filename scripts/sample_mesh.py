...
parser.add_argument('--igibson', action='store_true',
                    help='Whether to precess igibson dataset.')


def main(args):
    input_files = glob.glob(os.path.join(args.in_folder, '*.off'))
    if args.igibson == True:
        class_folders = [os.path.join(args.in_folder, class_f) for class_f in os.listdir(
            args.in_folder) if os.path.isdir(os.path.join(args.in_folder, class_f)) and class_f == 'basket']
        input_files = []
        for class_folder in class_folders:
            test_split_file = os.path.join(class_folder, 'test.lst')
            with open(test_split_file, 'r') as f:
                test_folders = f.read().split('\n')
            # obj_folders = [os.path.join(os.path.join(class_folder, obj_f)) for obj_f in os.listdir(
            #     class_folder) if os.path.isdir(os.path.join(class_folder, obj_f))]
            obj_folders = [os.path.join(os.path.join(class_folder, obj_f)) for obj_f in test_folders if obj_f != '' and os.path.isdir(os.path.join(class_folder, obj_f))]
            for obj_f in obj_folders:
                input_files.append(os.path.join(obj_f, 'mesh_watertight.off'))
                if not os.path.exists(input_files[-1]):
                    print("{} doesnot exits!!!".format(input_files[-1]))
                    exit(-1)

    print("Total mesh num: {}".format(len(input_files)))
    mesh_cnt = 0
    ...
    print("process {} meshes".format(mesh_cnt))
...
def process_path(in_path, args):
    ...
    if args.igibson:
        modelname = os.path.dirname(in_path).split('/')[-1]
    ...
    if not args.resize:
        ...
        scale = (bbox[1] - bbox[0]) / (1 - args.bbox_padding)
    ...
    if args.igibson == True:
        output_folder = os.path.dirname(in_path) + "/for_occnet"
        if not os.path.exists(output_folder):
            os.makedirs(output_folder)
        args.pointcloud_folder = output_folder
        args.points_folder = output_folder
        args.mesh_folder = output_folder
        # args.voxels_folder = output_folder
...
def export_mesh(mesh, modelname, loc, scale, args):
    ...
    if args.igibson:
        filename = os.path.join(args.mesh_folder, "model.off")
    if not args.overwrite and os.path.exists(filename):
